...
const createHelperCategory = (name, helpers, imports) => {
  const subHelpers = helpers
    .filter(h => h.helperType === 'direct')
    .map(h => createIndividualHelper(h.name, h.inputType, h.outputType));
  const subCategories = helpers
    .filter(h => h.helperType === 'category')
    .map(
      h =>
...
export declare interface ${utils.uppercaseFirstLetter(name)} {
    ${subHelpers.join('\n    ')}
    ${subCategories
      .map(h => `${h.name}: ${utils.uppercaseFirstLetter(h.name)}`)
      .join('\n    ')}
}`;
};
...
const createHelpersObject = topLevelCategories => {
  return topLevelCategories
    .map(
      c =>
        `${c
          .substring(0, c.length - 6)
          .toLowerCase()}: ${utils.uppercaseFirstLetter(c)}`,
    )
    .join(';\n    ');
};
